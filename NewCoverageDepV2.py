...
root = tk.Tk()
sprayFile = fd.askopenfilenames()
# sprayFile = tuple('r' + ele1 for ele1 in sprayFile)
...
fileName = ''+str(date.today().isoformat())+'_protection_rate_for_different_sizes'+'_for_'+'swab_spray_and_mask'
# #===================== END INPUT PARAMETERS ==========================

raw_data = pandas.read_csv(inputFile, delim_whitespace=True, comment='%', header=0)
raw_data = raw_data.dropna()
raw_data = raw_data.to_numpy()
spray_data = pandas.read_csv(str(sprayFile[0]), delim_whitespace=True, comment='%', header=0)
...
# fig = plt.subplot()
ax = plt.subplot(2,1,1)
ax.bar(size+1.2, (depos - deposnew)/depos*100, width=0.2, color = 'k', ec = 'k', align='center')
ax.bar(size+1.4, (depos - deposswab)/depos*100, width=0.2, color = 'darkgrey', ec = 'k', align='center')
ax.bar(size+1.6, (depos - deposspray)/depos*100, width=0.2, color = 'w', ec = 'k', align='center')
legen = {'Mask':'k','Swab':'w', 'Spray':'darkgrey'}
labels = list(legen.keys())
handles = [plt.Rectangle((0,0),1,1, color=legen[label]) for label in labels]
...
plt.xticks(xvalue,xlabels,align = 'center')
