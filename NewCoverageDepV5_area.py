...
from Coverage_Functions import filter_Spray_RefinedV2
import os
...
for k in range(Numsit):
    ...
    # Spray = Spray[Spray['z'] > min(Spray['z']) + 0.2]
    ##
    ...
    Numofsize = len(size)
    # hist = (sortdata['size']).hist(bins = 200)


    SpID = filter_Spray_RefinedV2(Spray,sortdata)
    ##No Mask Analysis
    ...
    for i in range(0,Numofsize):
        tepdata = SpID.loc[SpID['size'] == size[i]]
    ...
    # swabsort = sortdata[sortdata['z'] > coveragelenx]
    sprayswab = SpID.loc[(SpID['y'] > coveragelen) | (SpID['z'] > coveragelenx)]
    # sprayswab = SpID[SpID['z'] > coveragelenx]
    ...
    #Swab + Spray Analysis
    for i in range(0,Numofsize):
        tepdata = sprayswab.loc[sprayswab['size'] == size[i]]
    ...
    for i in range(1,Numofsize):
        if size[i] < 4:
            ...
            tepdata = sortdata.loc[sortdata['size'] == size[i]]
            dropsize = int(((size[i] - 1) /2000 * 0.50 + 0.25) * tepdata.shape[0])  ##Mask #1
            # dropsize = int(((size[i] - 1) /20 * 0.24 + 0.76) * tepdata.shape[0])  ##Mask #2
            # dropsize = int(((size[i] - 1)/20 * 0.70) * tepdata.shape[0]) ##Mask #3
            # dropsize = int(((size[i] - 1) /20 - 1) * -0.08 + 0.44) * tepdata.shape[0])  ##Mask #4
            drop_indices = np.random.choice(tepdata.index, dropsize, replace=False)
    ...
    # for i in range(0,Numofsize):
    #     tepdata = partnew.loc[partnew['size'] == size[i]]
    #     deposnew[i] = total_area_Data_Refined(inputmesh, tepdata,myIndex, False, False,threviral)
    ...
    ##Mask + Spray Analysis
    # SpIDmask = filter_Spray_RefinedV2(Spray,partnew)

    #Mask + Spray + Swab
    # SpIDmaskswab = SpIDmask[SpIDmask['y'] > coveragelen]
    ...
    #     deposswabnew[i] = total_area_Data_Refined(inputmesh, tepdata,myIndex, False, False,threviral)
    ...
    # for i in range(0,Numofsize):
    #     tepdata = swabpartnew.loc[swabpartnew['size'] == size[i]]
    #     deposspraynew[i] = total_area_Data_Refined(inputmesh, tepdata,myIndex, False, False,threviral)

    Max = total_area_Data_Refined(inputmesh, sortdata, myIndex, show3D, export3Dmesh,threviral) #Total depos No Mask as control
    Maxspray = total_area_Data_Refined(inputmesh, SpID,myIndex, False, False,threviral) #Spray
    Maxswab = total_area_Data_Refined(inputmesh, swabsort, myIndex, show3D, export3Dmesh,threviral) #Swab
    MedSS = total_area_Data_Refined(inputmesh, sprayswab, myIndex, show3D, export3Dmesh,threviral) #Swab+Spray
    MaxM = total_area_Data_Refined(inputmesh, partnew, myIndex, show3D, export3Dmesh,threviral) #Mask
    # MedSM = total_area_Data_Refined(inputmesh, SpIDmask, myIndex,show3D, export3Dmesh,threviral) #Mask+Spray
    # Minswab = total_area_Data_Refined(inputmesh, swabpartnew, myIndex, show3D, export3Dmesh,threviral) #Swab+Mask
    # Min = total_area_Data_Refined(inputmesh, SpIDmaskswab, myIndex, show3D, export3Dmesh,threviral) #Swab+Spray+Mask

    # print('viral particle at outlet has ' + len(viralout) + ' Particles \n'
    #       'Spray particle at outlet has ' + len(sprayout) + ' Particles')
    ...
    # fig = plt.subplot()
    ax = plt.subplot(2,1,1)
    ax.bar(size/100-0.2, (depos - deposnew)/depos*100, width=0.2, color = 'k', ec = 'k', align='center')
    ax.bar(size/100, (depos - deposswab)/depos*100, width=0.2, color = 'darkgrey', ec = 'k', align='center')
    ax.bar(size/100+0.2, (depos - deposspray)/depos*100, width=0.2, color = 'w', ec = 'k', align='center')
    ...
    ax.set_title('Protection rate between different situations',fontsize = 15)
    plt.xlabel('Protection Method')
    plt.ylabel('Protection Rate(%)')
    nameOfFig = str(int(spray_data[0,k*Npara+5]))+str(int(spray_data[0,k*Npara+6])) + filename
